...
function positiveNumber(value, name) {
	...
	if (!Number.isFinite(number) || number < 0) {
		throw httpError(400, "VALIDATION_ERROR", `${name} must be a valid positive number.`);
	}
	return number;
}
...
async function ownedVenue(venueId, ownerId, client = db) {
	...
	if (result.rowCount === 0) throw httpError(404, "VENUE_NOT_FOUND", "Venue not found.");
...
async function ownedEvent(eventId, ownerId, client = db) {
	...
	if (result.rowCount === 0) throw httpError(404, "EVENT_NOT_FOUND", "Event not found.");
	return result.rows[0];
}
...
async function eventDetail(idOrSlug, publicOnly = true) {
	...
	if (event.rowCount === 0) throw httpError(404, "EVENT_NOT_FOUND", "Event not found.");
	const value = event.rows[0];
	const [images, lineup, pricing] = await Promise.all([
		db.query("SELECT id, image_url AS \"imageUrl\", alt_text AS \"altText\", position FROM event_images WHERE event_id = $1 ORDER BY position", [value.id]),
		db.query("SELECT id, name, role, image_url AS \"imageUrl\", position FROM event_lineup WHERE event_id = $1 ORDER BY position", [value.id]),
		db.query(`SELECT MIN(price) AS \"minPrice\", MAX(price) AS \"maxPrice\", COUNT(*) FILTER (WHERE status = 'available')::int AS \"availableSeats\" FROM seats WHERE event_id = $1`, [value.id]),
	]);
	return { ...value, images: images.rows, lineup: lineup.rows, pricing: pricing.rows[0] };
}
...
async function venueDetail(idOrSlug, publicOnly = true) {
	...
	const [images, events] = await Promise.all([
		db.query("SELECT id, image_url AS \"imageUrl\", alt_text AS \"altText\", position FROM venue_images WHERE venue_id = $1 ORDER BY position", [value.id]),
		db.query(`SELECT id, title, slug, category, hero_image_url AS \"heroImageUrl\", starts_at AS \"startsAt\", ends_at AS \"endsAt\", status FROM events WHERE venue_id = $1 ${publicOnly ? "AND status = 'published'" : ""} ORDER BY starts_at ASC`, [value.id]),
	]);
...
router.get("/events", async (req, res, next) => {
	try {
		...
		if (search) add("(e.title ILIKE ? OR e.summary ILIKE ? OR v.name ILIKE ?)", `%${search}%`);
		if (search) { const i = params.length; where[where.length - 1] = `(e.title ILIKE $${i} OR e.summary ILIKE $${i} OR v.name ILIKE $${i})`; }
		...
		if (minPrice !== undefined) add("EXISTS (SELECT 1 FROM seats s WHERE s.event_id = e.id AND s.price >= ?)", Number(minPrice));
		if (maxPrice !== undefined) add("EXISTS (SELECT 1 FROM seats s WHERE s.event_id = e.id AND s.price <= ?)", Number(maxPrice));
...
router.get("/events/:idOrSlug/seats", async (req, res, next) => {
	try {
		...
		await db.query("UPDATE seats SET status = 'available', hold_expires_at = NULL WHERE event_id = $1 AND status = 'held' AND hold_expires_at < NOW()", [event.id]);
		const seats = await db.query(`SELECT id, zone_code AS "zoneCode", label, price, status FROM seats WHERE event_id = $1 ORDER BY zone_code, label`, [event.id]);
		return ok(res, { event: { id: event.id, title: event.title, currency: event.currency }, seats: seats.rows });
	} catch (error) { return next(error); }
});

router.get("/events/:idOrSlug", async (req, res, next) => {
	try { return ok(res, await eventDetail(req.params.idOrSlug)); } catch (error) { return next(error); }
});
...
router.get("/venues", async (req, res, next) => {
	try {
		...
		if (search) { params.push(`%${search}%`); where.push(`(v.name ILIKE $${params.length} OR v.city ILIKE $${params.length})`); }
		...
		const result = await db.query(
			...
			FROM venues v WHERE ${where.join(" AND ")} ORDER BY v.name`, params);
		return ok(res, result.rows);
	} catch (error) { return next(error); }
});

router.get("/venues/:idOrSlug", async (req, res, next) => {
	try { return ok(res, await venueDetail(req.params.idOrSlug)); } catch (error) { return next(error); }
});
...
router.get("/me/tickets", ...authenticated, async (req, res, next) => {
	try {
		await db.query("UPDATE seats SET status = 'available', hold_expires_at = NULL WHERE status = 'held' AND hold_expires_at < NOW()");
		await db.query("UPDATE tickets SET status = 'expired' WHERE status = 'pending' AND expires_at < NOW()");
		const tickets = await db.query(
			...
			WHERE t.customer_clerk_id = $1 GROUP BY t.id, e.id, v.id ORDER BY t.created_at DESC`, [req.authUserId]);
...
router.post("/tickets", ...authenticated, async (req, res, next) => {
	try {
		...
		if (seatIds.length === 0 || !seatIds.every((id) => typeof id === "string")) {
			throw httpError(400, "VALIDATION_ERROR", "At least one seatId is required.");
		}
		const ticket = await db.transaction(async (client) => {
			await client.query("UPDATE seats SET status = 'available', hold_expires_at = NULL WHERE id = ANY($1::uuid[]) AND status = 'held' AND hold_expires_at < NOW()", [seatIds]);
			const seats = await client.query(
				`SELECT s.id, s.event_id, s.price, s.status, e.currency, e.status AS event_status
				 FROM seats s JOIN events e ON e.id = s.event_id WHERE s.id = ANY($1::uuid[]) FOR UPDATE`, [seatIds]);
			if (seats.rowCount !== seatIds.length || new Set(seats.rows.map((seat) => seat.event_id)).size !== 1) {
				throw httpError(400, "INVALID_SEATS", "Seats must exist and belong to the same event.");
			}
			if (seats.rows.some((seat) => seat.status !== "available" || seat.event_status !== "published")) {
				throw httpError(409, "SEATS_UNAVAILABLE", "One or more selected seats are no longer available.");
			}
			const subtotal = seats.rows.reduce((sum, seat) => sum + Number(seat.price), 0);
			const created = await client.query(
				...
				[seats.rows[0].event_id, req.authUserId, seats.rows[0].currency, subtotal]);
			for (const seat of seats.rows) {
				await client.query("INSERT INTO ticket_seats (ticket_id, seat_id, unit_price) VALUES ($1, $2, $3)", [created.rows[0].id, seat.id, seat.price]);
			}
			await client.query("UPDATE seats SET status = 'held', hold_expires_at = NOW() + INTERVAL '15 minutes' WHERE id = ANY($1::uuid[])", [seatIds]);
...
router.get("/me/organizer/summary", ...authenticated, async (req, res, next) => {
	try {
		const summary = await db.query(
			...
			(SELECT COALESCE(SUM(t.total), 0) FROM events e JOIN tickets t ON t.event_id = e.id WHERE e.owner_clerk_id = $1 AND t.status = 'paid') AS revenue`, [req.authUserId]);
...
router.get("/me/venues", ...authenticated, async (req, res, next) => {
	try {
		const venues = await db.query(
			...
			FROM venues v WHERE v.owner_clerk_id = $1 ORDER BY v.created_at DESC`, [req.authUserId]);
		return ok(res, venues.rows);
	} catch (error) { return next(error); }
});

router.post("/me/venues", ...authenticated, upload.single("image"), async (req, res, next) => {
	try {
		const body = req.body;
		const imageUrl = (await uploadImage(req.file, "tickify/venues")) || body.imageUrl || null;
		const venue = await db.query(
			...
			[req.authUserId, required(body.name, "name"), createSlug(body.name), required(body.address, "address"), required(body.city, "city"), body.countryCode || null, body.latitude === undefined ? null : Number(body.latitude), body.longitude === undefined ? null : Number(body.longitude), body.description || null, imageUrl, positiveNumber(required(body.capacity, "capacity"), "capacity"), body.rating === undefined ? 0 : positiveNumber(body.rating, "rating"), body.audioSystem || null, body.lightingSystem || null, body.stageAreaSqm === undefined ? null : positiveNumber(body.stageAreaSqm, "stageAreaSqm"), body.status || null],
		);
		return ok(res, venue.rows[0], 201);
	} catch (error) { return next(error); }
});

router.patch("/me/venues/:id", ...authenticated, upload.single("image"), async (req, res, next) => {
	try {
		...
		const venue = await db.query(
			...
			[req.params.id, req.authUserId, body.name || null, body.address || null, body.city || null, body.countryCode || null, body.description || null, imageUrl, body.capacity === undefined ? null : positiveNumber(body.capacity, "capacity"), body.rating === undefined ? null : positiveNumber(body.rating, "rating"), body.audioSystem || null, body.lightingSystem || null, body.stageAreaSqm === undefined ? null : positiveNumber(body.stageAreaSqm, "stageAreaSqm"), body.status || null, body.latitude === undefined ? null : Number(body.latitude), body.longitude === undefined ? null : Number(body.longitude)],
		);
...
router.delete("/me/venues/:id", ...authenticated, async (req, res, next) => {
	try {
		await ownedVenue(req.params.id, req.authUserId);
		await db.query("DELETE FROM venues WHERE id = $1 AND owner_clerk_id = $2", [req.params.id, req.authUserId]);
...
router.get("/me/events", ...authenticated, async (req, res, next) => {
	try {
		const events = await db.query(
			...
			WHERE e.owner_clerk_id = $1 GROUP BY e.id, v.id ORDER BY e.starts_at DESC`, [req.authUserId]);
		return ok(res, events.rows);
	} catch (error) { return next(error); }
});

router.post("/me/events", ...authenticated, upload.single("image"), async (req, res, next) => {
	try {
		const body = req.body;
		await ownedVenue(required(body.venueId, "venueId"), req.authUserId);
		const imageUrl = (await uploadImage(req.file, "tickify/events")) || body.heroImageUrl || null;
		const event = await db.transaction(async (client) => {
			const created = await client.query(
				...
				[req.authUserId, body.venueId, required(body.title, "title"), createSlug(body.title), required(body.category, "category"), body.badgeText || null, body.summary || null, body.description || null, imageUrl, body.doorsAt || null, required(body.startsAt, "startsAt"), body.endsAt || null, body.currency || "USD", body.status || "draft"]);
			const seats = parseArray(body.seats);
			for (const seat of seats) {
				if (!seat || !seat.zoneCode || !seat.label) throw httpError(400, "VALIDATION_ERROR", "Each seat needs zoneCode and label.");
				await client.query("INSERT INTO seats (event_id, zone_code, label, price) VALUES ($1, $2, $3, $4)", [created.rows[0].id, seat.zoneCode, seat.label, positiveNumber(seat.price, "seat price")]);
			}
			return created.rows[0];
		});
...
router.patch("/me/events/:id", ...authenticated, upload.single("image"), async (req, res, next) => {
	try {
		...
		const event = await db.query(
			...
			[req.params.id, req.authUserId, body.venueId || null, body.title || null, body.category || null, body.badgeText || null, body.summary || null, body.description || null, imageUrl, body.doorsAt || null, body.startsAt || null, body.endsAt || null, body.currency || null, body.status || null]);
...
router.delete("/me/events/:id", ...authenticated, async (req, res, next) => {
	try {
		await ownedEvent(req.params.id, req.authUserId);
		await db.query("DELETE FROM events WHERE id = $1 AND owner_clerk_id = $2", [req.params.id, req.authUserId]);
		return res.status(204).end();
	} catch (error) { return next(error); }
});
